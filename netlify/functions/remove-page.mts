 ...
 *
 * Called by the X button on the library page. The library itself sits behind
 * the library-gate edge function, but that gate covers "/" and "/pages.json"
 * only — NOT this path — so the same cookie is verified here. Without that,
 * anyone who found this URL could take a client's page offline.
 *
 ...
 *
 * The Blobs state is deleted too. Leaving it would strand the data, and worse:
 ...
 *
 * Needs LIBRARY_PASSWORD (shared with the edge gate) and GITHUB_TOKEN (a
...
// Web Crypto rather than node:crypto, matching library-gate.ts — the two have
// to agree on the cookie value, so they should compute it the same way.
...
/** Must mirror state.mts exactly, or we would clear the wrong store. */
function stateStore() {
...
export default async (req: Request): Promise<Response> => {
  ...
  // The edge gate does not cover /api/*, so the cookie is checked here.
  ...
  const repo = Netlify.env.get("UO_ONBOARDING_REPO") ?? "unorthodox-digital/uo-onboarding";
  const branch = Netlify.env.get("UO_ONBOARDING_BRANCH") ?? "main";
  ...
  try {
    ...
    // a live page if the commit then failed.
    try {
      await stateStore().delete(slug);
    } catch (err) {
      // The page is already gone; a stale blob is untidy, not broken. Republishing
      // this slug would inherit it, so it is worth a log to find later.
